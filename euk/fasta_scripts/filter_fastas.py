...
def filter_fasta_complex(infasta_path, outfasta_path, euk_keep_list, prok_sp_list, prok_delimiter="-"):
	euk_regex = "EP\d+_\S+_P\d+"
	out_records = []
	for record in SeqIO.parse(infasta_path, "fasta"):
		seq_id = record.id
		if re.match(euk_regex, seq_id):
			if seq_id in euk_keep_list:
				out_records.append(record)
		else:
...
def filter_fastas_keep_complex_read_list(euk_list_dir, indir, outdir, prok_sp_list, list_ext=".txt"):
	for fasta in listdir_nohidden(indir):
		name = fasta.split(".")[0]
		print(name)
		infasta_path = indir + fasta
		...
		try:
			...
			filter_fasta_complex(infasta_path, outfasta_path, euk_keep_list, prok_sp_list, prok_delimiter="-")
	return outdir
...
outdir="/Users/vl18625/work/euk/markers_euks/nina_markers/01_07_23/archaea/faa_unfiltered/ae_81_markers_11_03_24/faa/"
...
filter_fastas_keep_complex_read_list(euk_list_dir, indir, outdir, archaea_keep_list, list_ext=".txt")
