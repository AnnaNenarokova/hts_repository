...
def rename_seqs_eukprot(fasta_path, out_tsv_path=False):
    if not out_tsv_path:
        out_tsv_path= fasta_path[:-6] + ".tsv"
    out_fasta = fasta_path[:-6] + "_renamed.fasta"
    results = []
...
def rename_many_seqs(fasta_dir, out_tsv_path):
    for fasta_file in listdir_nohidden(fasta_dir):
        ...
        rename_seqs_eukprot(fasta_path, out_tsv_path)
    return 0

fasta_dir="/Users/anna/work/euk_local/eukprot_proteomes/original_fastas/"
out_tsv_path="/Users/anna/work/euk_local/eukprot_proteomes/proteome_id_info.tsv"
