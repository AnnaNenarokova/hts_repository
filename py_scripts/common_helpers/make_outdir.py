...
def name_outdir(f, workdir=False, endcut=6):
	name = file_from_path(f, endcut=endcut)
	if workdir: outdir = workdir + name + '/'
	else: outdir = file_from_path(f, folder=True) + name + '/'
	return outdir

def make_outdir(f, workdir=False, endcut=6):
	outdir = name_outdir(f, workdir, endcut=endcut)
