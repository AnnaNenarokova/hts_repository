...
fasta = SeqIO.parse('/home/kika/programs/blast-2.5.0+/bin/cam_spades_12.fasta', 'fasta')
nt_out = open('/home/kika/MEGAsync/Chlamydomonas/cam_12/cam_pt_nt.txt', 'w')
aa_out = open('/home/kika/MEGAsync/Chlamydomonas/cam_12/cam_pt_aa.txt', 'w')
err_out = open('/home/kika/MEGAsync/Chlamydomonas/cam_12/cam_pt_errors.txt', 'w')
result_handle = open('/home/kika/MEGAsync/Chlamydomonas/cam_12/cam_cre_pt_blast.xml')
			...
			if best.hsps[0].expect > 0.01:
				...
				for hsp in best.hsps:
					if frame == hsp.frame[1]:
						...
						errors.append(record.query.split('_')[3])
						if frame in [1, 2, 3]:
							min_sstart = best.hsps[0].sbjct_start
							max_send = best.hsps[0].sbjct_end
						else:
							...
							max_send = best.hsps[0].sbjct_start
				if frame in [1, 2, 3]:
					result[record.query.split('_')[3]] = [min_sstart, max_send, frame, best.hit_id, 
						record.query_length, min_qstart, max_qend]
				else:
					result[record.query.split('_')[3]] = [max_send, min_sstart, frame, best.hit_id, 
						record.query_length, min_qstart, max_qend]
		except:
