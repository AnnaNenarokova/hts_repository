...
def trim_trimc (file_fw, file_rv, outdir, threads = 4, trimc_dir = False):
...
def trim_bbduk (file_fw, file_rv, outdir, threads = 4, bbduk_dir = False):
	if not bbduk_dir: bbduk_dir = '/home/anna/bioinformatics/bioprograms/Trimmomatic-0.32/'
	bbduk_out = outdir + 'bbduk_out/'
	if not os.path.exists(bbduk_out):
	    os.makedirs(bbduk_out)

	trimlog = bbduk_out +'trimlog'
	paired_out_fw = bbduk_out + 'paired_out_fw' + '.fastq'
	unpaired_out_fw = bbduk_out + 'unpaired_out_fw' + '.fastq'
	paired_out_rv = bbduk_out + 'paired_out_rv' + '.fastq'
	unpaired_out_rv = bbduk_out + 'unpaired_out_rv' + '.fastq'

	adapters_file = bbduk_dir + 'adapters/'+ "TruSeq3-PE-2.fa"

	trimmomatic = ['java', '-jar', bbduk_dir + 'trimmomatic-0.32.jar']
	trim_options = ['PE', '-threads', str(threads), '-phred33', '-trimlog', trimlog, file_fw, file_rv, 
					paired_out_fw, unpaired_out_fw, paired_out_rv, unpaired_out_rv,
					'ILLUMINACLIP:'+ adapters_file + ':2:30:10', 'LEADING:20', 'TRAILING:20', 'SLIDINGWINDOW:4:20',  
					'MAXINFO:40:0.8', 'MINLEN:20' ] 
...
def spades_assemble(outdir, test=False, spades_dir=False, file_fw=False, file_rv=False, trimc_out=False, memory=6):
		...
		if trimc_out:
			...
			for key in files:
				...
				spades_options = ['-1', files['PEfw'], '-2', files['PErv'], '-s', files['UPfw'], '-s', files['UPrv'], 
								  '-o', spades_out, '-m '+ str(memory), '--careful']
				spades_assemble= [spades] + spades_options

		elif file_fw and file_rv:
			spades_options = ['-o', spades_out, '-m '+ str(memory), '--careful']
...
def scaffold_abacas (contigs, reference, outdir, abacas_dir=False):
...
def prokka_annotate (sequence, outdir, prokka_dir=False):
	...
	prokka_options = ['--centre', 'XXX', '--kingdom', 'Bacteria', '--genus', 'Escherichia', '--species', 'coli', '--outdir', prokka_out, '--force', sequence]
	prokka_annotate = [prokka] + prokka_options
	call(prokka_annotate)
	return prokka_out

def handle_HTS (file_fw, file_rv, outdir, only_trim=False, only_assemble=False, threads=8):
	if not only_assemble: 
		trimc_out = trim_trimc(file_fw, file_rv, outdir, threads)
	else: 
		trimc_out = outdir + 'trimc_out/'
...
def handle_HTSes (workdir, file_fw = False, file_rv = False, HTS_dir = False, HTSes = False, only_trim = False, only_assemble = False):
	if file_fw and file_rv:
		...
		for fw, rv in HTSes:
			...
			if not os.path.exists(outdir): os.makedirs(outdir)
			if not only_trim: 
				handle_HTS (file_fw, file_rv, workdir)
			else: 
				handle_HTS (file_fw, file_rv, workdir, only_trim = True)
...
handle_HTSes(workdir, file_fw, file_rv)
